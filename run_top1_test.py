"""
TEST: Top-1 focused loss vs standard KL-only.
Addresses the T1 gap: standard FRR gets 48% T1 but 65% T10.
Can a T1-focused loss close that gap?
"""
...
from ultracompress.top1_loss import top1_match_loss, combined_distillation_loss
...
configs = [
    ("KL-only (baseline)", "kl"),
    ("KL + Top1 CE (alpha=0.3)", "top1_03"),
    ("Combined (KL+CE+Margin)", "combined"),
]

results = {}
for name, loss_type in configs:
    print(f"\n{'='*60}")
    print(f"  {name}")
    print(f"{'='*60}")
    ...
    for step in range(STEPS):
        ...
        elif loss_type == "top1_03":
            loss = top1_match_loss(student_logits, teacher_logits, T, alpha=0.3)
        elif loss_type == "combined":
            loss = combined_distillation_loss(student_logits, teacher_logits, T)
    ...
    results[name] = {'top1': t1, 'top10': t10}
    del model; gc.collect(); torch.cuda.empty_cache()

print(f"\n{'='*60}")
print("TOP-1 LOSS RESULTS")
print("=" * 60)
for name, r in results.items():
    print(f"  {name:40s}: T1={r['top1']*100:.0f}% T10={r['top10']*100:.0f}%")
print("Done!")
