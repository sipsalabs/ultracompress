"""Quick rotation engine test on GPU 1."""
...
STEPS = 15000
print("=" * 60)
print("ROTATION ENGINE TEST: 87K DNA vs 7.3M transformer")
print("=" * 60)
...
for n_planes in [64, 128, 256]:
    print(f"\n{'='*60}")
    print(f"  Rotation Engine (n_planes={n_planes}, 28 cycles)")
    print(f"{'='*60}")
    model = RotationEngine(1024, n_planes=n_planes, n_cycles=28, vocab_size=151936,
                           embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
    dna = model.dna_size()
    compression = teacher_layers / dna['total_dna']
    print(f"  DNA: {dna['total_dna']:,} params ({compression:.0f}x)")
    ...
    print(f"  FINAL: T1={t1*100:.0f}% T10={t10*100:.0f}% @ {compression:.0f}x DNA")
