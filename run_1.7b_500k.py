"""1.7B 500K TRAINING — Push quality toward 80%+.
100K gave 67%. Quality never plateaued. 500K is the real test."""
...
config = ModelConfig(n_layers=28, n_heads=16, n_kv_heads=8, hidden_size=2048, intermediate_size=6144, vocab_size=151936, head_dim=128)
...
model = FractalModel(hidden_dim=2048, n_heads=16, n_scales=4, iters_per_scale=7,
    vocab_size=151936, ff_mult=1,
    embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"FRR: {trainable:,} ({teacher_layers/trainable:.1f}x)")
opt = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=3e-4, weight_decay=0.01)
...
for step in range(STEPS):
    ...
    tokens = torch.randint(100, 50000, (2, 32), device=device)
    with torch.no_grad():
    ...
    if step % 50000 == 0 or step == STEPS - 1:
        t1, t10 = eval_model(model, n=100)
        marker = " <-- NEW BEST" if t10 > best_t10 else ""
        if t10 > best_t10: best_t10 = t10
        print(f"  Step {step}/{STEPS}: loss={loss.item():.4f} T1={t1*100:.0f}% T10={t10*100:.0f}% ({time.time()-t0:.0f}s){marker}")
t1, t10 = eval_model(model, n=200)
print(f"\nFINAL (200 samples): T1={t1*100:.0f}% T10={t10*100:.0f}%")
print(f"100K was 67%. 500K = {t10*100:.0f}%")
torch.save(model.state_dict(), 'frr_1.7b_500k_best.pt')
print("Done!")
