"""
LONG TRAINING: 50K steps on best config to test if 55% T10 is a training-time ceiling.
If quality keeps climbing past 10K steps, training time is the bottleneck.
If it plateaus, the architecture has hit its capacity limit at 0.6B.
"""
...
print("Building FRR (n_heads=16, ff_mult=1, 50K steps)...")
model = FractalModel(
...
print(f"Training {STEPS} steps (5x longer than usual)...")
t0 = time.time()
for step in range(STEPS):
    ...
    if step % 5000 == 0 or step == STEPS - 1:
        ...
        print(f"  Step {step}/{STEPS}: loss={loss.item():.4f} T1={t1*100:.0f}% T10={t10*100:.0f}% ({time.time()-t0:.0f}s)")

t1, t10 = eval_model(model, n=200)
print(f"\nFINAL (200 samples): T1={t1*100:.0f}% T10={t10*100:.0f}%")
print(f"10K baseline: ~55% T10")
print(f"50K result: {t10*100:.0f}% T10 ({'improved' if t10 > 0.56 else 'plateaued'})")
print("Done!")
