"""
PHM ON BEST CONFIG: Test hypercomplex multiplication with n_heads=16, ff_mult=1.
PHM gave 50% T10 at 4x fewer params on the weaker config.
On the best config with 50K steps, could reach 60%+ at 168x compression.
"""
...
# Test PHM at different step counts
for steps in [10000, 30000]:
    print(f"\n{'='*60}")
    print(f"  PHM n_heads=16, ff_mult=1, {steps} steps")
    print(f"{'='*60}")

    # Build FRR with PHM block instead of standard block
    model = FractalModel(
    ...
    print(f"  Trainable: {trainable:,} (compression: {compression:.1f}x)")
    ...
    print(f"  FINAL: T1={t1*100:.0f}% T10={t10*100:.0f}% @ {compression:.0f}x compression")
