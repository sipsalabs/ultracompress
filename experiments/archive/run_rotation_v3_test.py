"""Rotation V3 test: learned planes + causal scan + input-dependent angles."""
import lib.unbuffered
import torch, sys, os, time
import torch.nn.functional as F
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from ultracompress.inference import ModelConfig, MiniTransformer
from ultracompress.rotation_v3 import RotationV3

device = 'cuda'
STEPS = 15000
print("=" * 60)
print("ROTATION V3: Learned + Causal + Dynamic angles")
print("=" * 60)
...
model = RotationV3(1024, n_planes=64, n_cycles=28, vocab_size=151936,
                   use_causal=True, use_input_angles=True,
                   embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"  Trainable: {trainable:,}")
print(f"  V1 baseline: 44-46% T10")
...
print(f"  V1: 44-46%. V2 hub: 44%. V3: {t10*100:.0f}%")
print("Done!")
