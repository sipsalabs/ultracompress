"""
SEED JOINT TRAINING — Fix for the sequential failure.

Sequential (Phase 1 core, Phase 2 corrections) FAILED — corrections
disrupted the core. Solution: train EVERYTHING together from step 0.

Core + modulation + corrections all learn simultaneously.
The corrections learn to COMPLEMENT the core as it trains,
not patch it after the fact.
"""
...
print("=" * 60)
print("SEED JOINT TRAINING: Core + Corrections together")
print("=" * 60)
...
for rank in [0, 16, 32, 64]:
    print(f"\n{'='*60}")
    print(f"  JOINT SEED: rank={rank}, {STEPS} steps")
    print(f"{'='*60}")
    ...
    compression = teacher_layers / sizes['total_seed']
    print(f"  Seed: {sizes['total_seed']:,} params ({compression:.1f}x)")

    # ALL params train together from step 0
    ...
    for step in range(STEPS):
        ...
        # Ramp CE weight over training
        ce_weight = min(0.3, step / STEPS * 0.3)
        loss = (1 - ce_weight) * kl + ce_weight * ce
    ...
    results[f'joint_rank_{rank}'] = {'top1': t1, 'top10': t10, 'compression': compression}

    del model, opt; gc.collect(); torch.cuda.empty_cache()

print(f"\n{'='*60}")
print("JOINT SEED RESULTS")
print("=" * 60)
print(f"  Sequential baseline (rank 0): 55% T10 @ 60x")
print(f"  Sequential rank 16: 54% T10 @ 53x (corrections HURT)")
for name, r in results.items():
    marker = " <-- BETTER!" if r['top10'] > 0.55 else ""
    print(f"  {name:20s}: T1={r['top1']*100:.0f}% T10={r['top10']*100:.0f}% @ {r['compression']:.0f}x{marker}")
print("Done!")
