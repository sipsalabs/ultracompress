"""
TEST: Controller hypernetwork vs static modulation.
Compare: FRR with static gamma/beta vs FRR with input-dependent controller.

If controller beats static, it's the quality breakthrough we need.
"""
...
from ultracompress.controller import ControlledFRR
...
# 1. Static modulation baseline (standard FRR)
model = FractalModel(1024, 16, 4, 7, 151936, 1,
                     embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
results['Static'] = train_and_eval(model, "Static FRR (baseline)")
del model; gc.collect(); torch.cuda.empty_cache()

# 2. Controller with bottleneck=64
model = ControlledFRR(1024, 16, 4, 7, 151936, 1, controller_bottleneck=64,
                      embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
results['Controller-64'] = train_and_eval(model, "Controller FRR (bottleneck=64)")
del model; gc.collect(); torch.cuda.empty_cache()

# 3. Controller with bottleneck=128
model = ControlledFRR(1024, 16, 4, 7, 151936, 1, controller_bottleneck=128,
                      embed_weight=embed_w, lm_head_weight=lm_head_w, norm_weight=norm_w).to(device)
results['Controller-128'] = train_and_eval(model, "Controller FRR (bottleneck=128)")
del model; gc.collect(); torch.cuda.empty_cache()

print(f"\n{'='*60}")
print("CONTROLLER RESULTS")
print("=" * 60)
print(f"  {'Config':<25} {'T10':>5} {'Params':>12} {'Compression':>12}")
for name, r in results.items():
    print(f"  {name:<25} {r['top10']*100:>4.0f}% {r['params']:>12,} {r['compression']:>11.0f}x")
print(f"\n  Baseline reference (50K steps): 63% T10")
print("Done!")
