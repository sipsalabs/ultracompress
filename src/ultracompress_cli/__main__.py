...
Commands: list, pull, info, bench, version, demo.
"""
...
def _banner() -> None:
    console.print()
    console.print(
        "[bold cyan]UltraCompress[/bold cyan] "
...
@click.option(
    "--revision", type=str, default=None, help="Specific Hub revision (commit SHA or tag) to pull."
)
def cmd_pull(model_id: str, output: str | None, revision: str | None) -> None:
    ...
    console.print(
        f"[cyan]->[/cyan] Pulling [bright_white]{model_id}[/bright_white] "
...
def cmd_info(path: str) -> None:
    ...
    if meta is None:
        console.print(
            f"[red]X[/red] No ultracompress metadata found at [bright_white]{path}[/bright_white]"
        )
        console.print(
            "[dim]Is this a directory produced by `uc pull`? "
            "Or an ultracompress.json manifest?[/dim]"
        )
        sys.exit(1)
    summarize_artifact(meta, console, Path(path))
...
@click.option(
    ...
    help="Where to save lm-eval-harness result files.",
)
def cmd_bench(
    path: str, tasks: str, limit: int, batch_size: int, device: str, output_dir: str
) -> None:
    """Benchmark a compressed artifact on downstream tasks."""
    _banner()
    console.print(
        f"[cyan]->[/cyan] Benchmarking [bright_white]{path}[/bright_white] "
