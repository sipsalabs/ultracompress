...
near-lossless (~1% perplexity; lossy) 5-bit patent-pending compression reference artifacts.
...
The v0.2 native near-lossless 5-bit kernel path ships in Q3 2026, gated on patent
prosecution timing.
...
BASELINES = {
    ...
    "HQQ 4-bit g64": 0.563,
    "UltraCompress 5 bpw (near-lossless)": 0.625,  # 5 bits / 8 bits per byte
}

ULTRACOMPRESS_BPP = BASELINES["UltraCompress 5 bpw (near-lossless)"]


@dataclass
class FleetSavings:
    """Per-baseline savings vs UltraCompress 5 bpw (near-lossless)."""
...
def compute_savings(params_per_model: float, model_count: int,
    ...
    for name, bpp in BASELINES.items():
        if name == "UltraCompress 5 bpw (near-lossless)":
            continue
    ...
    return {
        ...
        "notes": [
            "Storage and egress savings reflect the on-disk artifact size (5 bpw near-lossless vs the baseline). "
            ...
            "gated on the quantized-runtime kernel path shipping with native near-lossless 5-bit inference. Use it as a v0.2 "
