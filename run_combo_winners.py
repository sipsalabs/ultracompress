"""
COMBO WINNERS: PHM + LoRA + PredCoding on the best config (n_heads=16, ff_mult=1).
Combines the three MEGA winners on the architecture that hits 56% baseline.

PHM gives 4x fewer params. LoRA gives +3% T10. PredCoding gives +7% T10.
Can we stack them?
"""
...
configs = [
    # (name, n_heads, ff_mult, use_lora, lora_rank)
    ("Baseline n16f1", 16, 1, False, 0),
    ("LoRA n16f1", 16, 1, True, 16),
    ("LoRA-32 n16f1", 16, 1, True, 32),
]

results = {}
for name, n_heads, ff_mult, use_lora, lora_rank in configs:
    print(f"\n{'='*60}")
    print(f"  {name}")
    print(f"{'='*60}")

    model = FractalModel(
        hidden_dim=1024, n_heads=n_heads, n_scales=4, iters_per_scale=7,
    ...
    print(f"  FINAL: T1={t1*100:.0f}% T10={t10*100:.0f}% ({compression:.1f}x)")
    results[name] = {'top1': t1, 'top10': t10, 'params': trainable, 'compression': compression}
...
for name, r in results.items():
    print(f"  {name:30s}: T10={r['top10']*100:.0f}% @ {r['compression']:.0f}x ({r['params']:,} params)")
