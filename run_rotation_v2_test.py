"""
ROTATION V2 TEST: Does the global hub break the 44% plateau?

V1: rotation + local mixing = 44% T10 (plateaued at all plane counts)
V2: rotation + local + GLOBAL HUB = should break plateau

The hub connects all positions through a 32-dim bottleneck.
Like a central nervous system. O(n), not O(n²).
"""
...
print("=" * 60)
print("ROTATION V2: Global hub breaks the 44% plateau?")
print("=" * 60)
...
print(f"\n{'='*60}")
print(f"  Rotation V2 (64 planes + global hub, 28 cycles)")
print(f"{'='*60}")
model = RotationEngine(1024, n_planes=64, n_cycles=28, vocab_size=151936,
...
print(f"  V1 baseline: 44% T10 (no hub)")
print(f"  Improvement from hub: {(t10*100 - 44):+.0f}%")
print("Done!")
