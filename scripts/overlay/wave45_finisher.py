...
def wait_for_both():
    ...
    stable_rounds = 0
    while True:
        cur = {OLMO: stable(OLMO), SMOL: stable(SMOL)}
        both_exist = cur[OLMO] > 0 and cur[SMOL] > 0
        both_stable = (cur[OLMO] == prev[OLMO]) and (cur[SMOL] == prev[SMOL])
        if both_exist and both_stable and stable_rounds >= 1:
            return
        if both_exist and cur == prev:
            stable_rounds += 1
        else:
            stable_rounds = 0
        prev = cur
        with open(ROOT / "logs" / "wave45_finisher.log", "a",
                  encoding="utf-8") as f:
            f.write(f"{time.strftime('%H:%M:%S')}  olmo={cur[OLMO]:>12}  "
                    f"smol={cur[SMOL]:>12}  stable_rounds={stable_rounds}\n")
        time.sleep(60)
