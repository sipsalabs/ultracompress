"""
HQ3 BREAKTHROUGH — Push past the T1/T10 plateau.

Key innovations vs HQ2:
1. DIRECT T1 LOSS: Cross-entropy on teacher's argmax (not just KL)
2. CONFIDENCE WEIGHTING: Upweight positions where teacher is confident
3. LARGER BODY: h=256 (4x more capacity than HQ2's h=128)
4. LONGER CONTEXT: SEQ_LEN=128 (2x HQ2)
5. MORE DATA: 500M tokens (5x HQ2)
6. DUAL-PHASE LOSS: Early training = broad KL (T10), late training = sharp CE (T1)

Loss = fkl + rkl_w*rkl + latent_w*latent + ce_w*ce_on_argmax + margin_w*margin
  where ce_w and margin_w increase as temperature drops (T1 focus in late training)

Warm-starts from h256 FSD body (best available for h=256).
Uses teacher's frozen norm + lm_head (proven in HQ2).
"""
...
WARMUP = 1000
H_INNER = args.h
TAG = args.tag or f'hq3_h{H_INNER}'
N_TEACHER_LAYERS = 28
DEVICE = args.device
T_START = 2.0
T_END = 1.0
...
print(f"TinyFRR-HQ3 BREAKTHROUGH: inner={H_INNER}  steps={STEPS}  tag={TAG}  device={DEVICE}")
print(f"  T: {T_START}->{T_END}  rkl_w={RKL_W}  latent_w={LATENT_W}  ce_w={CE_W}  margin_w={MARGIN_W}")
...
class TinyFRR_HQ3(nn.Module):
    """HQ3 = HQ2 architecture + direct T1 targeting.

    Same as HQ2: proj_in → FRR body → proj_out → teacher norm → teacher lm_head.
    All outputs use teacher's frozen norm + lm_head for maximum information transfer.
    """
...
# ==================== Warm-start ====================
# Prefer: hq2 h256 > FSD body > hq h256 > vanilla h256
WARM_CANDIDATES = [
...
# Also try loading body from FSD checkpoint if available
FSD_WARM = f'checkpoints_1.7b_tinyfrr_fsd_h{H_INNER}_r512/best.pt'
...
if WARM_PATH:
    ...
    warm_loaded = True
elif os.path.exists(FSD_WARM):
    # Load body weights from FSD (proj_in, inner.*, proj_out won't exist but body will)
    warm = torch.load(FSD_WARM, map_location=DEVICE, weights_only=False)
    sd = warm.get('state_dict', warm)
    # Map FSD keys to HQ3 keys (both share proj_in + inner.block/scale_gamma/etc.)
    filtered = {}
    for k, v in sd.items():
        if k.startswith('proj_in.') or k.startswith('inner.block.') or \
           k.startswith('inner.scale_gamma') or k.startswith('inner.scale_beta') or \
           k.startswith('inner.iter_scale'):
            filtered[k] = v
    missing, unexpected = student.load_state_dict(filtered, strict=False)
    print(f"  WARM START (body) from {FSD_WARM}  loaded={len(filtered)} missing={len(missing)}")
    warm_loaded = True
...
# ==================== Data / Opt ====================
# Use 500M tokens for more diversity
...
def ce_schedule(step):
    """CE and margin weights ramp up as temperature drops.
    Early training: focus on KL (broad T10 signal).
    Late training: add CE+margin (sharp T1 signal).
    """
    # Start ramping at 20% of training, full at 60%
    ramp_start = int(STEPS * 0.2)
    ramp_end = int(STEPS * 0.6)
    if step < ramp_start:
        return 0.1  # Small CE from the start (seed the T1 pathway)
    if step >= ramp_end:
        return 1.0
...
for step in range(start_step, STEPS):
    ...
    for micro in range(ACCUM):
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            ...
            # ---- Loss 3: Latent matching (proven in HQ2) ----
            tl = t_latent.float()
            sl = s_latent.float()
            ...
            # ---- Loss 4: DIRECT T1 — Cross-entropy on teacher argmax ----
            # This directly teaches the student to match teacher's top prediction
            t_argmax = t_logits_f.argmax(dim=-1)  # [B, S]

            # Confidence weighting: upweight positions where teacher is confident
            t_entropy = -(t_prob * t_logp).sum(-1)  # [B, S]
            # Low entropy = confident = weight more. Transform: w = 1/(1 + entropy)
            conf_weight = 1.0 / (1.0 + t_entropy)
            conf_weight = conf_weight / conf_weight.mean()  # normalize to mean=1

            # CE loss weighted by confidence
            ce_loss = F.cross_entropy(
                s_logits.view(-1, s_logits.size(-1)),
                t_argmax.view(-1),
                reduction='none',
            ).view(s_logits.size(0), s_logits.size(1))
            ce_loss = (ce_loss * conf_weight).mean()

            # ---- Loss 5: Margin ranking — teacher's #1 should beat runner-up ----
            s_logits_f = s_logits.float()
            s_at_t1 = s_logits_f.gather(-1, t_argmax.unsqueeze(-1)).squeeze(-1)  # [B, S]
            # Get student's max logit excluding teacher's top-1
            s_mask = s_logits_f.clone()
            s_mask.scatter_(-1, t_argmax.unsqueeze(-1), float('-inf'))
            s_runner_up = s_mask.max(dim=-1).values  # [B, S]
            margin_loss = F.relu(MARGIN_TARGET - (s_at_t1 - s_runner_up))
            margin_loss = (margin_loss * conf_weight).mean()

            # ---- Combined loss ----
            # CE and margin ramp up during training (ce_ramp: 0.1 → 1.0)
            loss = (fkl
                    + RKL_W * rkl
    ...
    if step % 100 == 0:
        ...
        t1_hit = int(accum_ce < 5.0)  # rough proxy
        print(f"  [{TAG}] step={step:6d}  loss={accum_loss:.4f}  fkl={accum_fkl:.4f}  "
              ...
              f"T={T:.2f}  ce_ramp={ce_ramp:.2f}  lr={lr:.5f}  ({time.time()-t0:.0f}s)", flush=True)
