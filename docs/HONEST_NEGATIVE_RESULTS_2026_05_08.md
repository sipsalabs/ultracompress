...
We catalogue these fourteen outcomes for the same reason we publish the positive ones: the only way someone reading our work can know the positive numbers are honest is if the failures are equally visible. The 2026-05-08 → 2026-05-09 arc produced an all-time tightest dense-decoder PPL ratio (Qwen3-1.7B-Base 1.0040×, JSON-traceable to `PPL_EVAL_qwen3-1_7b-base_2026_05_08.json`), an 18-architecture validation matrix, and 9 public `uc verify`-PASS HuggingFace artifacts. It also produced 14 distinct negative or failed outcomes — two SSM-correction approaches that did not work (items 1, 2), one runner crash that needed a code patch (3), one eval-harness CUDA assert that defeated us (4), four HF upload paths that hit residential-network or xet-token failures (5–9, partially overlapping), one published research hypothesis (base-vs-instruct quantization friendliness) that survived only one of three controlled tests (10), one parameter-saturation result that closes the door on rank-and-steps as a cheap path below 1.001× (11), one apples-to-apples refutation of the per-Linear adaptive bpw v1 hypothesis (12), one v2 marginal-win result whose statistical power is suspect pending a seed sweep (13), one catastrophic refutation of multi-pass cascade correction at constant param budget (14), and one catastrophic refutation of AWQ-style channel pre-scaling on GSQ + V18-C (15, the V4-A experiment landed today). The ratio of catalogued failures to published positives across the two-day arc is now roughly 15:9, and that is the ratio we want any external evaluator to use when assessing whether our positive numbers are real. They are.
