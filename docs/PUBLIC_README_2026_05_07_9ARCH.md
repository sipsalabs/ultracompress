...
The 9-architecture matrix as of 2026-05:
...
Streams the source model from disk one decoder layer at a time. Caches teacher hidden states. Trains a per-layer correction overlay. Writes a single `.uc` directory of layer-shaped artifacts. Peak VRAM bounded by one decoder layer + activations.
...
2. **Per-layer correction overlay training (Phase 2)** — for each layer in turn: load weights, apply 5-bit scalar quantization scalar quantization, wrap each Linear with a low-rank correction overlay (`y = (alpha * Wq + V@U) @ x + b`), train low-rank corrections via hidden-MSE for 200 steps with SVD warm-start.
