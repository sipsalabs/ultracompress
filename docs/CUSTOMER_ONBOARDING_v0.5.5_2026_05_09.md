# UltraCompress — Customer Onboarding (v0.5.5)
...
> **What changed in v0.5.5 (read this first).** Packs are now **self-contained**. The customer
> downloads ONE directory (~5-30 GB depending on model size, plus ~622 MB auxiliary for typical
> decoder vocab embedding tables) and gets a working model — no separate bf16 download, no
> external dependencies, no second `hf download` command. LayerNorm tensors, `embed_tokens`, and
> `lm_head` are all bundled into the pack. SHA-256 is still verifiable end-to-end against the
> manifest. Net effect on the customer experience: **3 commands instead of 5** to reproduce any
> published artifact, and Day 7 deliverables are now genuinely portable as a single-directory
> handoff.
...
Sipsa Labs, Inc. — `founder@sipsalabs.com` — `https://github.com/sipsalabs/ultracompress` — `https://pypi.org/project/ultracompress/0.5.5/`

---

## 0. What changed since v0.5.4 (read this once)

Two operational upgrades landed today (2026-05-09) on top of v0.5.4:

1. **Self-contained packs.** v0.5.5 packs ship LayerNorm + `embed_tokens` + `lm_head` inside the pack directory itself. The customer no longer needs the original bf16 checkpoint to run the compressed model — one `hf download` of the SipsaLabs pack is sufficient. SHA-256 verification still covers the full forward pass end-to-end against the manifest. Disk-cost: ~622 MB auxiliary for a typical decoder vocab on top of the compressed weights (small relative to the multi-GB compressed body).
2. **3-command repro path.** Reproduction of any published artifact is now `hf download && uc bench && uc verify`. The pre-v0.5.5 flow required pulling the original bf16 alongside the pack (5 commands). With self-contained packs, the comparison-to-baseline step is opt-in (Day 0 baseline measurement is still recommended but no longer required for reproduction itself).

Carried forward from v0.5.4 (still true in v0.5.5):

- `uc bench` ships in the CLI — single command that measures TTFT, steady-state TPS, and peak VRAM on any UC pack or HF bf16 checkpoint, on the customer's hardware.
- 22 transformer architectures validated end-to-end (dense 0.6B → 405B, MoE 47B → 235B, state-space). Full matrix in Appendix A.
- Hermes-3-Llama-3.1-405B in the public catalog as the first 405B-class artifact (compression complete; HF upload + PPL eval both in flight; honest PPL number publishes the moment eval completes).
...
Optional — measure your own baseline before kickoff. v0.5.5 ships `uc bench`;
running it against your bf16 model now gives you a clean comparison target for
our Day 7 delivery (same command we'll use, directly comparable JSON). Self-
contained packs in v0.5.5 mean the Day 7 reproduction itself doesn't require
the bf16 checkpoint, so this baseline step is a comparison nicety, not a
prerequisite.
...
- v0.5.5 of the public CLI shipped today — packs are now self-contained, so the
  Day 7 deliverable is a single-directory handoff:
...
Self-service benchmarking (v0.5.5): your custom artifact will benchmark with
...
Day 7 — same command, same JSON schema, same machine. v0.5.5 packs are self-
contained, so the deliverable is a single-directory handoff.
...
- Compressed pack: `<customer-short-name>-uc-v3-bpw5/` via [secure transfer]
  (self-contained v0.5.5 pack — LayerNorm + embed_tokens + lm_head bundled;
  ~0.31x source fp16 size + ~622 MB aux for vocab).
...
How to verify the delivery yourself — three commands, no bf16 download needed:

    pip install ultracompress==0.5.5 huggingface_hub[cli]
    uc bench   ./<customer-short-name>-uc-v3-bpw5 --device cuda:0 --out delivered_bench.json
...
Expected `delivered_bench.json` shape:

    {
      ...
      "uc_version": "0.5.5", "pack_version": "v3"
    }

Compare to `baseline_bench.json` from Day 0 if you measured one. Same machine,
same command, directly comparable — no Sipsa-side claims to take on faith.
...
```bash
pip install ultracompress==0.5.5 huggingface_hub[cli]   # v0.5.5 — self-contained packs
...
## 3. Measure on your own hardware: `uc bench`

Most useful command in the CLI. One invocation produces a directly comparable TTFT / TPS / peak-VRAM triple against any UC pack OR any HF bf16 checkpoint, on your target device.

```bash
# Baseline a bf16 model (Day 0 of any POC — optional in v0.5.5):
...
## 4. Reproduce a published artifact (3 commands, ~3 min)

Smallest fully-published artifact is Qwen3-1.7B-Base at ~1.1 GB. With v0.5.5 self-contained packs, reproduction is genuinely three commands:
...
That's it. No separate bf16 download, no second hf-hub command, no environment setup beyond `pip install`. The pack contains everything needed to run the model and verify the SHA-256 manifest end-to-end.

`uc verify` checks pack-format integrity, SHA-256 against `manifest.json`, all declared layers present, presence of bundled LayerNorm + `embed_tokens` + `lm_head` (new in v0.5.5), and a sample layer `W_base` reconstruction shape.
...
- `SipsaLabs/llama-3.1-70b-uc-v3-bpw5` (~44 GB) — uploaded last release (80 .uc files on HF; PPL eval pending — verify before customer use)
...
> `uc compress` ships in v0.6.0. In v0.5.5 the flow is a manual two-step.
...
# Step 2 — pack to v3 .uc (self-contained — bundles LayerNorm + embed_tokens + lm_head)
...
| `Missing embed_tokens` on a v0.4.x pack | Pre-v0.5.5 pack format (not self-contained) | Re-pack with v0.5.5 `pack_e2e_dir_v3` |
...
| `uc verify` fails on a freshly-packed dir | Almost always a v0.4.x pack format leak | Re-pack with v0.5.5 `pack_e2e_dir_v3` |
...
- **PyPI release notes:** https://pypi.org/project/ultracompress/0.5.5/
...
**The deal:** Sipsa compresses + verifies + benchmarks ONE customer-specified transformer model to 5 bpw v3 within 5 business days. You receive: the `.uc` pack (self-contained as of v0.5.5 — no separate bf16 download required), a `uc verify` PASS report, a `uc bench` JSON benchmark you can re-run yourself, and a one-page deployment guide. Acceptance gate: `uc verify` PASS + PPL ratio within 1.5% of baseline on your eval set. $2,500 on signature, $2,500 net 30 on delivery.
...
*End of guide. Last updated 2026-05-09 against v0.5.5. Supersedes v0.5.4 (2026-05-09).*
