...
# UltraCompress shared-block parameter dispatch (shared-block) evidence matrix

> **This is the shared-block parameter dispatch (shared-block) evidence matrix — architectural-compression evidence only.** It is **separate** from the Row-Overlay Quantization (RoQ) 2.798-bpw v0.1 reference-artifact benchmark in the project [README on GitHub](https://github.com/sipsalabs/ultracompress#readme). Do not compare retention numbers across methods as a single quality curve — RoQ is post-training quantization (shipping now); shared-block is architectural compression (v0.2, Q3 2026).

**Method:** shared-block parameter dispatch (USPTO 64/049,517, patent pending)
**Availability:** evidence now; product availability v0.2 (Q3 2026)
**Customer ship status:** not yet downloadable; pre-compressed reference models for shared-block parameter dispatch (shared-block) release in v0.2
...
|---|---|---|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| TinyLlama-1.1B | Llama-2 | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 1.1 | shared-block | v0.2 | 2.3988 | 1.6985x | 83.61% | 91.73% | 65.01% | 94.17% | 17.0142 | 28.8989 |
| OLMo-2-1B | OLMo-2 | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 1.485 | shared-block | v0.2 | 2.3906 | 1.7898x | 82.75% | 90.83% | 62.76% | 93.06% | 20.1537 | 36.0711 |
| SmolLM2-1.7B | SmolLM2 | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 1.812 | shared-block | v0.2 | 2.3906 | 1.8988x | 80.84% | 90.18% | 62.57% | 93.20% | 18.0321 | 34.2397 |
| Qwen3-1.7B | Qwen3 | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 1.7 | shared-block | v0.2 | 2.4017 | 1.788x | 84.65% | 90.68% | 64.04% | 93.88% | 33.21 | 59.4 |
| Mistral-7B-v0.3 | Mistral | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 7.248 | shared-block | v0.2 | 2.3942 | 1.6274x | 86.21% | 93.19% | 69.69% | 95.06% | 12.3569 | 20.1093 |
| Qwen3-8B | Qwen3 | shared-block parameter dispatch (shared-block) evidence | Not yet downloadable | 8.19 | shared-block | v0.2 | 2.3967 | 1.3859x | 91.85% | 95.83% | 73.56% | 96.98% | 20.6963 | 28.6829 |
...
## Cohort summary (shared-block parameter dispatch (shared-block) at the same operating point)
...
## Row-Overlay Quantization (RoQ) (separate experiment — referenced for context only)

The README headline numbers (95.63% median T1 retention, zero catastrophic failures, 30% smaller than NF4) are the **Row-Overlay Quantization (RoQ)** benchmark at **2.798 bpw**. RoQ and shared-block are different methods, different operating points, different bpw targets. Customer evaluations should pick a method based on use case:

- **Row-Overlay Quantization (RoQ) (shipping now)** — drop-in replacement for bitsandbytes / GPTQ / AWQ / HQQ. Pre-compressed reference artifacts roll out on Hugging Face Hub through April–May 2026.
- **shared-block parameter dispatch (shared-block) (v0.2)** — architectural compression layered on top of (or in place of) standard quantization. Higher absolute compression, narrower architecture support at v0.2 launch.
...
- **`Experiment family`** — which experimental method this row belongs to. All rows in this matrix are shared-block evidence.
...
- This is the shared-block parameter dispatch (shared-block) *evidence matrix* — architectural-compression evidence. (internal experiment).1 reference-artifact benchmarks are in the README. **Do not combine.**
