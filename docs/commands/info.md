...
|---|---|---|
| `<path>` | yes | Path to a directory or `ultracompress.json` file from a Sipsa-compressed pack (downloaded via `huggingface-cli download`) |
...
- **Path is a manifest but the referenced `model.safetensors` SHA mismatches** → prints with "verified ✗" warning but still exits 0 (re-download via `huggingface-cli download`)

## See also

- [Downloading a compressed model](pull.md) — download an artifact via `huggingface-cli`
