...
UltraCompress combines two complementary patent-pending compression components. Two USPTO provisional applications are on file covering this work; the specific method, codec geometry, and the procedure used to produce a compressed pack are NDA-gated.
...
## Low-bpw weight representation (shipping)
...
UltraCompress is a novel post-training weight representation. Across the 6-model benchmark cohort at 5 bits per weight:
...
## Architectural compression (roadmap)

A second, complementary line of work compresses the *architecture* itself — restructuring transformer blocks to retain expressive capacity at substantially fewer trainable parameters. Public detail on this component is intentionally limited until it ships. Public-safe architectural-compression evidence is at [evidence/matrix.md](../evidence/matrix.md). Release timing is gated on patent prosecution; early-access design partners can engage now via the [pilot program](../PILOT_PACKET.md).

## What "catastrophic failure" means

We use a published `T_cat` threshold: any cohort member whose perplexity ratio exceeds 10× the FP16 baseline is a catastrophic failure. HQQ at 2-bit and lower produces models that cross this threshold; UltraCompress at 5 bpw does not, in the cohort we tested. See [catastrophic-failures.md](catastrophic-failures.md).
...
| Codec operating-point parameters and internal structure | — | ✅ |
| Architectural-compression mechanism and specification | — | ✅ |
| Patent specifications (provisional filings on record) | — | ✅ when public record |
...
- A supplemental filing extends claim scope on the shipping codec; details available under NDA
- The architectural-compression component is on the roadmap, gated on patent prosecution timing
