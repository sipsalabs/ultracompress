...
We use a published `T_cat` threshold: any cohort member whose perplexity ratio exceeds 10× the FP16 baseline is a catastrophic failure. HQQ at 2-bit and lower produces models that cross this threshold; Track A at 2.798 bpw does not, in the cohort we tested. See [catastrophic-failures.md](catastrophic-failures.md).
