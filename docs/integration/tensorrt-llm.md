...
UltraCompress is patent-pending; two USPTO provisional applications are on file and claim text is available under NDA. NVIDIA TensorRT-LLM is a closed-source NVIDIA product. Our integration works at the **export-format** level — we produce a TensorRT-LLM-compatible engine — without requiring NVIDIA to integrate or license our methods.
